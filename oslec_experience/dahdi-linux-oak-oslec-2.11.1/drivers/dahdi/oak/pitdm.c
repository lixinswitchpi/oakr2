/*
 * SwitchPi Thunder module Driver for DAHDI Telephony interface.
 * This driver is based on Digium WCTDM driver and developed to support SwitchPi Thunder 4 ports board only,
...
//module reset pin, PA14, CS0 in DTS
#define RESET_CS 0
#define RESET_CS_H 1
#define RESET_CS_L 0
int wcnt;
volatile unsigned int *rt, *wt;

extern void codec_reset(void);
extern void __spi_write(unsigned char cmd, int cs);
extern unsigned char __spi_read(int cs);
extern int __readchunk ( void (*isr_callback)(u32 *read_samples, u32 *write_samples)) ;
extern void start_i2s_tx(void);
extern void stop_i2s(void);
...
static void reset_codec (void) {
	codec_reset();
}

static inline void wctdm_transmitprep(struct wctdm *wc, u32 *writesamples)
{
...
#endif
static inline void wctdm_receiveprep(struct wctdm *wc, u32 *readsamples)
{
...
static void ___spi_write(unsigned char reg, struct wctdm *wc) {
	int cs = wc->curcard;
	__spi_write(reg, cs);
}

static unsigned char ___spi_read(struct wctdm *wc) {
	unsigned char reg;
	int cs = wc->curcard;
	reg = __spi_read(cs);
	return reg;
...
static void __wctdm_setreg(struct wctdm *wc, int card, unsigned char reg, unsigned char value)
{
	...
	if (wc->modtype[card] == MOD_TYPE_FXO) {
		___spi_write(0x20, wc);
		___spi_write(reg & 0x7f, wc);
	} else {
		___spi_write(reg & 0x7f, wc);
	}
	___spi_write(value, wc);
}
...
static unsigned char __wctdm_getreg(struct wctdm *wc, int card, unsigned char reg)
{
	...
	__wctdm_setcard(wc, card);
	if (wc->modtype[card] == MOD_TYPE_FXO) {
		___spi_write(0x60, wc);
		___spi_write(reg & 0x7f, wc);
	} else {
		___spi_write(reg | 0x80, wc);
	}
	return ___spi_read(wc);
}
...
//static void work_set_hook_status_handler(struct work_struct *test)
static void work_set_hook_status_handler(void)
{
	struct wctdm *wc = gwc;
	int x;
	int mode;
...
void process_int_in_here(u32 *readsamples, u32 *writesamples);
void process_int_in_here(u32 *readsamples, u32 *writesamples)
{
	struct wctdm *wc = gwc;

	wctdm_receiveprep(wc, readsamples);
	wctdm_transmitprep(wc, writesamples);
...
static void fx_auto_detect(struct wctdm *wc) {
	...
	for (i = 0; i < NUM_CARDS; i++) {
		...
		___spi_write(0x60, wc);
		___spi_write(0x02, wc);
		reg = ___spi_read(wc);
		if (reg == 0x3) {
			wc->modtype[i] = MOD_TYPE_FXO;
			//printk("Found a fxo card on %d\n", i);
		}else {
			reset_spi(wc, 1);
			__wctdm_setcard(wc, i);
			___spi_write(0x01 | 0x80, wc);
			reg = ___spi_read(wc);
			if ((reg & 0xC0) == 0x80) {
				wc->modtype[i] = MOD_TYPE_FXS;
				//printk("Found a fxs card on %d\n", i);
			}
		}
	}
}
...
/*
static void fx_auto_detect1(void) {
	unsigned char reg = 0xff;
	int i;

	reset_codec();
	//write a dummy byte first
	spi_write_reg(0xff);
	udelay(500);
	for ( i=0; i<100; i++) {
		//spi_write_reg(0x81);
		reset_codec();
		spi_write_reg(0x60);
		spi_write_reg(0x02 & 0x7f);
		spi_read_reg(&reg);
		if (reg != 0x03) printk("error \n");
	}
	spi_read_reg(&reg);
	printk("read out values is %x\n", reg);
	if (reg == 0x03) {
		printk("found a fxo module\n");
	}
}
*/
void thunder_interrupt_processing(u32 *read_samples, u32 *write_samples) {
	wcnt ++;
	process_int_in_here(read_samples, write_samples);
}
...
static int wctdm_init_one(void)
{
	int res;
	struct wctdm *wc;
	...
	if (0) {
		...
		if (wc) {
			...
			wc->dev = thdev;
			wc->pos = x;
			wc->variety = "SwitchPi Thunder FXS/FXO";
			...
			//put a global tracks back to the wc
			gwc = wc;
			if (cardcount != 0) led_on();
			else led_off();
...
static int __init wcthunder_init(void)
{
	if (wctdm_init() != 0) {
		printk("wctdm_init error\n");
		return -1;
	}

	if (wctdm_init_one() != 0) {
		printk("wctdm_init_one error\n");
		return -1;
	}

	__readchunk (thunder_interrupt_processing);

    return 0;
}

static void __exit wcthunder_exit(void)
{
	stop_i2s();
	wctdm_release(gwc);
	printk("wc thunder exit\n");
}

module_init(wcthunder_init);
module_exit(wcthunder_exit);
